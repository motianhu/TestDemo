 ...
 */
public abstract class HorPagedView extends ViewGroup implements
    ...
    public HorPagedView(Context context) {
        this(context, null);
    }

    public HorPagedView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HorPagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mConfigScaleValue = context.getResources().getInteger(
                R.integer.config_scale_value) / 100f;
        mConfigAlphaValue = context.getResources().getInteger(
                R.integer.config_alpha_value) / 100f;
        mConfigScaleLimitValue = context.getResources().getInteger(
                R.integer.config_scale_limit_value) / 100f;
        mConfigAlphaLimitValue = context.getResources().getInteger(
                R.integer.config_alpha_limit_value) / 100f;
    ...
    /**
     * Note: this is a reimplementation of View.isLayoutRtl() since that is
    ...
    /**
     * Called during AllApps/Home transitions to avoid unnecessary work. When
     * that other animation ends, {@link #resumeScrolling()} should be called,
     * along with {@link #updateCurrentPageScroll()} to correctly set the final
     * state and re-enable scrolling.
     */
    void pauseScrolling() {
        mScroller.forceFinished(true);
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (DEBUG)
                Log.d(TAG, "computeScrollHelper mNextPage: " + mNextPage
                        + ", mCurrentPage: " + mCurrentPage);
            if (isLoop(getChildCount())) {
                if (mNextPage == -1) {
                    ...
                    mCurrentPage = Math.max(0,
                            Math.min(mNextPage, getPageCount() - 1));
                }
            } else {
                mCurrentPage = Math.max(0,
                        Math.min(mNextPage, getPageCount() - 1));
            }
            // mCurrentPage = Math.max(0, Math.min(mNextPage, getPageCount() -
            // 1));
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (child != null) {
                    ...
                    float scrollProgress = getScrollProgress(screenCenter,
                            child, i, mConfigAlphaLimitValue);
                    float alpha = Math.abs(scrollProgress);
                    alpha = Math.min(alpha, mConfigAlphaLimitValue);
                    child.setAlpha(alpha);

                    // scale
                    float scrollProgressScale = getScrollProgress(screenCenter,
                            child, i, mConfigScaleLimitValue);
                    float scale = mConfigScaleValue
                            - Math.abs(scrollProgressScale);
                    scale = Math.max(scale, mConfigScaleValue - 0.1f);
    ...
    protected int getChildOffset(int index) {
        ...
        int childCount = getChildCount();
        if (isLoop(childCount) && childOffsets != null
                && index == childOffsets.length) {
            return childOffsets[index - 1]
                    + getScaledMeasuredWidth(getPageAt(index - 1))
                    + mPageSpacing;
        } else if (isLoop(childCount) && childOffsets != null && index == -1) {
            return childOffsets[0] - getScaledMeasuredWidth(getPageAt(0))
                    - mPageSpacing;
        } else if (childOffsets != null && childOffsets[index] != -1) {
    ...
    protected int getRelativeChildOffset(int index) {
        int childCount = getChildCount();
        if (isLoop(childCount) && mChildRelativeOffsets != null
                && index == mChildRelativeOffsets.length) {
            final int padding = getPaddingLeft() + getPaddingRight();
            final int offset = getPaddingLeft()
                    + (getMeasuredWidth() - padding - getChildWidth(index - 1))
                    / 2;
            return offset;
        } else if (isLoop(childCount) && mChildRelativeOffsets != null
                && index < 0) {
            final int padding = getPaddingLeft() + getPaddingRight();
            final int offset = getPaddingLeft()
                    + (getMeasuredWidth() - padding - getChildWidth(0)) / 2;
            return offset;
        } else if (mChildRelativeOffsets != null
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                if (isLoop(childCount)
                    ...
                    drawLastView(canvas, screenCenter, drawingTime,
                            childCount - 1, -offset);

                    // the (childCount - 2) of view
                    if (leftScreen == rightScreen) {
                        drawLastNearView(canvas, screenCenter, drawingTime,
                                childCount - 2, -offset);
                    }

                    drawFirstView(canvas, drawingTime);

                } else if (isLoop(childCount)
                        && ((rightScreen == childCount - 1)
                                && (leftScreen == rightScreen - 1) || (leftScreen == rightScreen))) {
                    View view = null;
                    ...
                    // the second of view
                    if (leftScreen == rightScreen) {
                        drawLastNearView(canvas, screenCenter, drawingTime, 1,
                                offset);
                    }

                    drawFirstView(canvas, screenCenter, drawingTime,
                            childCount, offset);
                }

                // Clip to the bounds
                drawRect(canvas, leftScreen, rightScreen, drawingTime);
            }
        }
    }

    private void drawRect(Canvas canvas, final int leftScreen,
            final int rightScreen, final long drawingTime) {
        canvas.save();
        canvas.clipRect(getScrollX(), getScrollY(), getScrollX() + getRight()
                - getLeft(), getScrollY() + getBottom() - getTop());
    ...
    private void drawFirstView(Canvas canvas, final long drawingTime) {
        canvas.save();
    ...
    private void drawFirstView(Canvas canvas, int screenCenter,
            final long drawingTime, int childCount, int offset) {
        drawView(canvas, screenCenter, drawingTime, 0, offset, childCount);
    }

    private void drawLastNearView(Canvas canvas, int screenCenter,
            final long drawingTime, int page, int offset) {
        drawView(canvas, screenCenter, drawingTime, page, offset, page);
    }

    private void drawLastView(Canvas canvas, int screenCenter,
            final long drawingTime, int page, int offset) {
        drawView(canvas, screenCenter, drawingTime, page, offset, -1);
    }

    private void drawView(Canvas canvas, int screenCenter,
            final long drawingTime, int page, int offset, int processIndex) {
        canvas.save();
        canvas.translate(offset, 0);
        View view = getChildAt(page);
        setDrawParams(screenCenter, processIndex, view);
        drawChild(canvas, view, drawingTime);
        canvas.restore();
    }

    private void setDrawParams(int screenCenter, int page, View view) {
        float scrollProgress = getScrollProgress(screenCenter, view, page,
                mConfigAlphaLimitValue);
        float alpha = Math.abs(scrollProgress);
        alpha = Math.min(alpha, mConfigAlphaLimitValue);
        view.setAlpha(alpha);

        // scale
        float scrollProgressScale = getScrollProgress(screenCenter, view, page,
                mConfigScaleLimitValue);
        float scale = mConfigScaleValue - Math.abs(scrollProgressScale);
        scale = Math.max(scale, mConfigScaleValue - 0.1f);
        view.setScaleX(scale);
        view.setScaleY(scale);
    }

    private boolean isLoop(int childCount) {
        return mIsLoop && childCount > 1;
    }
    ...
    protected float getScrollProgress(int screenCenter, View v, int page,
        ...
        float scrollProgress = delta * limit / (totalDistance * 1.0f);
    ...
    int getPageNearestToCenterOfScreen() {
        ...
        for (int i = 0; i < childCount; ++i) {
            ...
            // if (Math.abs(screenCenter) < halfChildWidth) {
            // if (i == 0 && Math.abs(screenCenter) <= (halfChildWidth / 2)) {
            // distanceFromScreenCenter = 0;
            // } else if (i == (childCount - 1)
            // && Math.abs(screenCenter) > (halfChildWidth / 2)) {
            // distanceFromScreenCenter = 0;
            // }
            // }

            if (DEBUG) {
                Log.d(TAG, "i : " + i + ", halfChildWidth: " + halfChildWidth
                        / 2 + ",  getChildOffset(i): " + getChildOffset(i)
        ...
        if (DEBUG) {
            Log.d(TAG, "minDistanceFromScreenCenterIndex : "
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (DEBUG)
            Log.d(TAG, "snapToPage.getChildOffset(): "
                    + getChildOffset(whichPage) + ", whichPage: " + whichPage);
        if (DEBUG)
    ...
    protected void snapToPage(int whichPage, int duration) {
        if (DEBUG)
            Log.d(TAG, "snapToPage.whichPage: " + whichPage);
        if (isLoop(getChildCount())) {
            whichPage = Math.max(-1, Math.min(whichPage, getPageCount()));
        } else {
            whichPage = Math.max(0, Math.min(whichPage, getPageCount() - 1));
    ...
    protected View getScrollingIndicator() {
        // We use mHasScrollIndicator to prevent future lookups if there is no
        // sibling indicator
        // found
        if (mHasScrollIndicator && mScrollIndicator == null) {
            ViewGroup parent = (ViewGroup) getParent();
            if (parent != null) {
                mScrollIndicator = null;// (View)
                                        // (parent.findViewById(R.id.paged_view_indicator));
     ...
     * should stretch to fill its space on the track or not.
     */
    ...
    private void updateScrollingIndicatorPosition() {
        ...
        float offset = Math.max(0f,
                Math.min(1f, (float) scrollPos / mMaxScrollX));
        if (isRtl) {
            offset = 1f - offset;
        }
